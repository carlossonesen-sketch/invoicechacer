/**
 * API route to send due date invoice email
...
try {
  ...
  if (process.env.NODE_ENV !== "production") {
    console.warn("[SEND DUE EMAIL] Firebase Admin init failed at module load (will retry at runtime):", error instanceof Error ? error.message : "Unknown error");
  }
}

/**
 * Send due date invoice email
 */
export async function POST(request: NextRequest) {
  try {
    ...
    if (!existingEvent.empty) {
      return NextResponse.json(
        { error: "Due date invoice email already sent", alreadySent: true },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json({
      success: true,
      message: "Due date invoice email sent",
    });
  } catch (error) {
    console.error("[SEND DUE EMAIL] Error:", error);
